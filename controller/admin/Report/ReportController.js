...
const AdminReportView = async (req, res) => {
	try {
		...
		let start_date = req.query.start_date ?  new Date(req.query.start_date) : '';
		let end_date = req.query.end_date ? new Date(req.query.end_date) : '';

		if (trx_type === "plus") {

			if(start_date && end_date){
				const data = await TransactionsModels.find({ user_name: { $regex: search, $options: "i" }, trx_type: "+", remark: { $regex: remark, $options: "i" }, "created_at": {"$gte": start_date, "$lt": end_date} });
				res.status(201).json({
					success: true,
					data: data,
